...
export const listCalendar = createServerFn({ method: "GET" })
  ...
  .inputValidator((d: unknown) => z.object({ factoryId: z.string().uuid(), year: z.number().int(), month: z.number().int().min(1).max(12) }).parse(d))
...
export const generateDefaultMonth = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const pad = (n: number) => String(n).padStart(2, "0");
    const lastDay = new Date(data.year, data.month, 0).getDate();
    const rows: { factory_id: string; day: string; is_workday: boolean; created_by: string; updated_by: string }[] = [];
    ...
    // upsert
    const { error } = await supabase.from("work_calendar_days").upsert(rows, { onConflict: "factory_id,day" });
    if (error) throw new Error(error.message);
